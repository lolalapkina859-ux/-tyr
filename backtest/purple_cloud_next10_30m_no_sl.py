...
# Second independent basket. HYPE is omitted because Binance USD-M archive may not provide it reliably.
SYMBOLS=["ZECUSDT","LTCUSDT","BCHUSDT","DOTUSDT","UNIUSDT","AAVEUSDT","NEARUSDT","APTUSDT","ARBUSDT","OPUSDT"]
...
def summarize(x,name):
 ...
 return {"symbol":name,"trades":len(x),"wins":len(w),"losses":len(l),"winrate":round(100*len(w)/len(x),2) if len(x) else 0,"sum_trade_returns_pct":round(x.return_pct.sum(),4),"avg_return_per_signal_pct":round(x.return_pct.mean(),4),"avg_winner_pct":round(w.return_pct.mean(),4) if len(w) else 0,"avg_loser_pct":round(l.return_pct.mean(),4) if len(l) else 0,"max_winner_pct":round(x.return_pct.max(),4),"max_loser_pct":round(x.return_pct.min(),4),"avg_hold_hours":round(x.bars.mean()*0.5,2)}
...
def main():
 ...
 x.to_csv(out/"purple_cloud_next10_30m_no_sl_detail.csv",index=False); pd.DataFrame(summaries).to_csv(out/"purple_cloud_next10_30m_no_sl_summary.csv",index=False); print(pd.DataFrame(summaries).to_string(index=False))
