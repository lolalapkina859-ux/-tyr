...
# Current LIVE portfolio: BASE14 + FLOW
BASE15 = list(eng.BASE14) + ['FLOWUSDT']
# Current account equity observed at latest successful LIVE deployment log.
# Can be overridden manually in Actions with PC_TEST_START_BALANCE.
...
def main():
    ...
    try:
        for size in SIZES:
            ...
            eng.NOTIONAL = size
            name = f"BASE15_BAL{START_BALANCE:.2f}_NOTIONAL{int(size)}"
    ...
    summary.to_csv(OUT / 'pc_base15_current_balance_size_sweep_summary.csv', index=False)
    pd.concat(trades, ignore_index=True).to_csv(OUT / 'pc_base15_current_balance_size_sweep_trades.csv', index=False)
