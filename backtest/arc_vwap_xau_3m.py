...
def main():
    ...
    pd.DataFrame(rows).to_csv("backtest/data/arc_vwap_xau_3m_summary.csv",index=False)
    lot_rows=[]
    for name,b,se in [("RAW_FLIPS","raw_buy","raw_sell"),("SESSION_VWAP_CONFIRMED_ONLY","conf_buy","conf_sell")]:
        r=backtest_lots(s,b,se); r["variant"]=name; lot_rows.append(r)
    pd.DataFrame(lot_rows).to_csv("backtest/data/arc_vwap_xau_3m_lot002_summary.csv",index=False)
    print("\n0.02 LOT RESULTS (1 lot = 100 oz):")
    print(pd.DataFrame(lot_rows).to_string(index=False))
    s[["time","open","high","low","close","volume","raw_buy","raw_sell","conf_buy","conf_sell"]].to_csv("backtest/data/arc_vwap_xau_3m_signals.csv",index=False)
