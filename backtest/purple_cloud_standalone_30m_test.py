...
def run_mode(asset, symbol, mode):
    ...
    for i in range(len(d)):
        ...
        if pos is not None:
            ...
            if opposite or timeout:
                ...
                raw_r = ((exit_price-pos["entry"])/pos["risk"] if pos["side"] == "LONG"
                         else (pos["entry"]-exit_price)/pos["risk"])
                rows.append({"asset":asset,"mode":mode,"side":pos["side"],
                             "entry_time":pos["time"],"exit_time":bar["time"],
                             "entry":pos["entry"],"exit":exit_price,"initial_risk":pos["risk"],
                             "realized_r":raw_r,"bars":i-pos["i"],
    ...
    if pos is not None and len(d):
        ...
        rows.append({"asset":asset,"mode":mode,"side":pos["side"],
                     "entry_time":pos["time"],"exit_time":bar["time"],
                     "entry":pos["entry"],"exit":exit_price,"initial_risk":pos["risk"],
                     "realized_r":raw_r,"bars":len(d)-1-pos["i"],"exit_reason":"END_OF_DATA"})
    return pd.DataFrame(rows)


def summarize(x, asset, mode):
    if x.empty:
        return {"asset":asset,"mode":mode,"trades":0,"wins":0,"losses":0,"winrate":0,
                "total_r":0,"avg_r":0,"median_r":0,"avg_bars":0}
    wins = int((x["realized_r"] > 0).sum())
    losses = int((x["realized_r"] < 0).sum())
    return {"asset":asset,"mode":mode,"trades":len(x),"wins":wins,"losses":losses,
...
def main():
    ...
    for asset,symbol in ASSETS.items():
        for mode in ("NORMAL","STRONG"):
            ...
            all_rows.append(x); sums.append(summarize(x,asset,mode))
    ...
    detail.to_csv(out/"purple_cloud_standalone_30m_detail.csv",index=False)
    summary.to_csv(out/"purple_cloud_standalone_30m_summary.csv",index=False)
    print("=== PURPLE CLOUD STANDALONE 30M ===")
