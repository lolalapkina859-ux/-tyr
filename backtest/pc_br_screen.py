...
def run_period(name,start,end):
    ...
    if d is None or len(d)<60:
        return {'period':name,'symbol':SYMBOL,'candles':0 if d is None else len(d),'trades':0,'sum_return_pct':None,'winrate_pct':None,'status':'insufficient_data'}, pd.DataFrame()
    pc=purple_cloud(d)
    sig=[]
    ...
    return {'period':name,'symbol':SYMBOL,'candles':len(d),'trades':len(t),'sum_return_pct':float(t.return_pct.sum()) if len(t) else 0.0,'winrate_pct':float((t.return_pct>0).mean()*100) if len(t) else 0.0,'status':'ok'},t
...
def main():
    ...
    summary.to_csv(f'{OUT}/pc_br_screen_summary.csv',index=False)
    (pd.concat(ts,ignore_index=True) if ts else pd.DataFrame()).to_csv(f'{OUT}/pc_br_screen_trades.csv',index=False)
