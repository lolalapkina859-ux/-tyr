...
def main() -> None:
    p = argparse.ArgumentParser(description='Fast post-filter: 15M LIMIT signals + 1H direction')
    ...
    p.add_argument('--out', default='backtest_btc_15m_1h_filter.csv')
    args = p.parse_args()
    ...
    # Strictly closed 1H candle only: no look-ahead.
    # Direction = close vs previous close + close vs EMA20.
    ...
    for _, trade in base.iterrows():
        ...
        if pd.isna(t):
            continue
        # 1H bar whose CLOSE is <= signal time.
    ...
    print('\n=== 15M LIMIT BASELINE (OB 1.10) ===')
    print(summarize(base))
    print('\n=== 15M LIMIT + CLOSED 1H DIRECTION FILTER ===')
