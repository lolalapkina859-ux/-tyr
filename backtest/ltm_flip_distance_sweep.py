...
def main():
        ...
        for pn,st,en in PERIODS:
            ...
            for m in DISTANCES:
                ...
                for s,d in raw.items():
                    x=signals(d,m); data[s]=x[["time","high","low","close","pc_buy","pc_sell"]]
    ...
    pd.DataFrame(rows).to_csv("backtest/data/ltm_flip_distance_sweep_summary.csv",index=False)
    if bys: pd.concat(bys,ignore_index=True).to_csv("backtest/data/ltm_flip_distance_sweep_by_symbol.csv",index=False)
