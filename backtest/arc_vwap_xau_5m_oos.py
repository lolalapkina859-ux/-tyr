...
def floating_risk_test(d,buycol,sellcol,lots=(0.001,0.002)):
    rows=[]
    for lot in lots:
        qty=lot*OZ_PER_LOT
...
def main():
    ...
    pd.DataFrame(rows).to_csv("backtest/data/arc_vwap_xau_5m_oos_summary.csv",index=False)
    lot_rows=[]
    for name,b,se in [("RAW_FLIPS","raw_buy","raw_sell"),("SESSION_VWAP_CONFIRMED_ONLY","conf_buy","conf_sell")]:
        r=backtest_lots(s,b,se); r["variant"]=name; lot_rows.append(r)
    pd.DataFrame(lot_rows).to_csv("backtest/data/arc_vwap_xau_5m_oos_lot002_summary.csv",index=False)
    ...
    risk.to_csv("backtest/data/arc_vwap_xau_5m_oos_floating_risk.csv",index=False)
    print("\nVWAP CONFIRMED FLOATING RISK — START BALANCE $150-ish:")
    print(risk.to_string(index=False))
    s[["time","open","high","low","close","volume","raw_buy","raw_sell","conf_buy","conf_sell"]].to_csv("backtest/data/arc_vwap_xau_5m_oos_signals.csv",index=False)
