...
# Fresh candidate basket; excludes the 20 coins already screened.
SYMBOLS=['TRXUSDT','FILUSDT','ATOMUSDT','ETCUSDT','ICPUSDT','INJUSDT','RENDERUSDT','TIAUSDT','SEIUSDT','WIFUSDT','PEPEUSDT','FETUSDT','TAOUSDT','ENAUSDT','PENDLEUSDT']
...
def stat(x,s,p):
 ...
 return {'symbol':s,'period':p,'trades':len(x),'winrate':round(100*len(w)/len(x),2) if len(x) else 0,'sum_return_pct':round(x.return_pct.sum(),4),'avg_signal_pct':round(x.return_pct.mean(),4) if len(x) else 0,'avg_winner_pct':round(w.return_pct.mean(),4) if len(w) else 0,'avg_loser_pct':round(l.return_pct.mean(),4) if len(l) else 0,'best_pct':round(x.return_pct.max(),4) if len(x) else 0,'worst_pct':round(x.return_pct.min(),4) if len(x) else 0}
...
def main():
 ...
 piv['both_positive']=(piv.MAR_JUN>0)&(piv.JUN_SEP>0); piv['six_month_sum_pct']=piv[['MAR_JUN','JUN_SEP']].sum(axis=1); piv['worst_period_pct']=piv[['MAR_JUN','JUN_SEP']].min(axis=1); piv=piv.sort_values(['both_positive','worst_period_pct','six_month_sum_pct'],ascending=False)
 out=Path('backtest/data'); out.mkdir(parents=True,exist_ok=True); sm.to_csv(out/'pc_candidates_6m_split_summary.csv',index=False); piv.to_csv(out/'pc_candidates_6m_split_ranking.csv',index=False)
 if details: pd.concat(details,ignore_index=True).to_csv(out/'pc_candidates_6m_split_detail.csv',index=False)
